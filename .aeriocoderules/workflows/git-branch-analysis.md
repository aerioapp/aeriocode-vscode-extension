...
**If the expected line count is greater than 500 lines, use the file-based approach:**
   ```shell
   git branch --show-current > aeriocode-git-analysis.temp && echo "=== STATUS ===" >> aeriocode-git-analysis.temp && git status --porcelain >> aeriocode-git-analysis.temp && echo "=== COMMIT MESSAGES ===" >> aeriocode-git-analysis.temp && git log main..HEAD --oneline >> aeriocode-git-analysis.temp && echo "=== CHANGED FILES ===" >> aeriocode-git-analysis.temp && git diff main --name-only >> aeriocode-git-analysis.temp && echo "=== FULL DIFF ===" >> aeriocode-git-analysis.temp && git diff main >> aeriocode-git-analysis.temp
   ```
   ...
   ```shell
   rm aeriocode-git-analysis.temp
   ```
