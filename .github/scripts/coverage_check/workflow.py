...
from .extraction import compare_coverage, run_coverage
from .github_api import generate_comment, post_comment, set_github_output
from .util import file_exists, get_file_size, list_directory, log, run_command
...
def run_extension_coverage(branch_name=None):
    ...
    # Run coverage tests
    ext_cov = run_coverage(["xvfb-run", "-a", "npm", "run", "test:coverage"], file_path, "extension")
...
def run_webview_coverage(branch_name=None):
    ...
    try:
        ...
        web_cov = run_coverage(["npm", "run", "test:coverage"], os.path.join("..", file_path), "webview")
    finally:
...
def generate_warnings(
    ...
    if ext_decreased and ext_diff > 1.0:
        warnings.append(
            f"Extension coverage decreased by more than 1% ({ext_diff}%). Consider adding tests to cover your changes."
        )

    if web_decreased and web_diff > 1.0:
        warnings.append(
            f"Webview coverage decreased by more than 1% ({web_diff}%). Consider adding tests to cover your changes."
        )
...
def process_coverage_workflow(args):
    ...
    try:
        ...
        comment = generate_comment(
            base_ext_cov,
            pr_ext_cov,
            str(ext_decreased).lower(),
            ext_diff,
            base_web_cov,
            pr_web_cov,
            str(web_decreased).lower(),
            web_diff,
        )
