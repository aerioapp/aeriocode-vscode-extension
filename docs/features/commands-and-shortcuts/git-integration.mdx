...
Aeriocode's Git integration brings AI assistance directly to your version control workflow. Generate commit messages without leaving your editor.
...
2. Click the robot icon in the Source Control view or run the "Generate Commit Message with Aeriocode" command
3. Aeriocode analyzes your changes and generates a descriptive commit message
...
When you use Aeriocode's commit message generation feature, here's what happens behind the scenes:

1. Aeriocode retrieves the current Git diff using `getWorkingState()`
...
This process uses your current Aeriocode API configuration, so the quality of the generated messages matches your chosen AI model.
...
1. When you trigger the command:
    - Aeriocode gets the current diff
...
Next time you're struggling to write a good commit message, try using Aeriocode's commit message generation. You'll save time and improve your version control workflow with AI assistance right where you need it.
