...
Aeriocode's terminal integration lets you bring your terminal output directly into your conversations with Aeriocode. Instead of copying and pasting error messages or command results, you can send them to Aeriocode with a simple right-click in the terminal.

## Right-Click Terminal Integration

When you're working in the VSCode terminal and see output you want to discuss with Aeriocode:

1. Right-click in the terminal
2. Select "Add to Aeriocode" from the context menu
3. The terminal output is immediately sent to the Aeriocode chat panel
...
Instead of switching context to the Aeriocode chat panel and typing a description of the problem, you can send the terminal output directly to Aeriocode with just a couple of clicks.

Alternatively, you can use the [`@terminal`](/features/at-mentions/terminal-mentions) mention to send the full terminal output to Aeriocode.
...
When you use the right-click terminal integration, Aeriocode:
...
This gives Aeriocode the full context it needs to provide accurate help with terminal-related issues.

## Behind the Scenes

The terminal integration uses a clever technique to capture terminal output:

1. When you trigger the integration, Aeriocode:
...
-   **Use terminal integration for error messages**: When you encounter an error in the terminal, sending it to Aeriocode often results in faster resolution than trying to describe the error.
...
-   **Combine terminal outputs with file mentions**: After sending terminal output to Aeriocode, you can enhance your question by mentioning relevant files using the @ mentions feature.
...
Next time you're staring at a cryptic error message in your terminal, try using Aeriocode's terminal integration instead of copying and pasting. You'll get more accurate help because Aeriocode can see the complete terminal context with proper formatting.
...
<Tip>
	**Quick Fix**: Most terminal issues can be resolved by switching to bash in the Aeriocode settings and increasing the shell
