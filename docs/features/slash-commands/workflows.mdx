...
Workflows allow you to define a series of steps to guide Aeriocode through a repetitive set of tasks, such as deploying a service or submitting a PR.

To invoke a workflow, type `/[workflow-name.md]` in the chat.

## How to Create and Use Workflows

Workflows live alongside [Aeriocode Rules](/features/aeriocode-rules). Creating one is straightforward:

1. Create a markdown file with clear instructions for the steps Aeriocode should take
...
-   Leverage Aeriocode's [built-in tools](/exploring-aeriocodes-tools/aeriocode-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
...
Let's walk through a real example of reviewing PR #3627 which fixes the thinking mode calculation for Aeriocode models.
...
3. Aeriocode actually supports a much larger thinking budget (up to 128000 tokens)
...
1. The PR correctly fixes the thinking mode budget calculation for Aeriocode models
2. It properly implements the thinkingConfig.maxBudget property (64000)
...
3. Let Aeriocode handle everything else

My workflow uses the `gh` command-line tool and Aeriocode's built in `ask_followup_question` to:
...
-   If I say "yes," Aeriocode automatically approves the PR with the `gh` command
