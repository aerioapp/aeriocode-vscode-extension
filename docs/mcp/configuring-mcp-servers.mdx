...
Utilizing MCP servers will increase your token usage. Aeriocode offers the ability to restrict or disable MCP server functionality as desired.

1. Click the "MCP Servers" icon in the top navigation bar of the Aeriocode extension.
2. Select the "Installed" tab, and then Click the "Advanced MCP Settings" link at the bottom of that pane.
3. Aeriocode will open a new settings window. find `Aeriocode>Mcp:Mode` and make your selection from the dropdown menu.
...
Settings for all installed MCP servers are located in the `aeriocode_mcp_settings.json` file:

1. Click the MCP Servers icon at the top navigation bar of the Aeriocode pane.
...
_Example of MCP Server config in Aeriocode (STDIO Transport)_
...
After configuring an MCP server, Aeriocode will automatically detect available tools and resources. To use them:

1. Type your request in Aeriocode's conversation window
2. Aeriocode will identify when an MCP tool can help with your task
3. Approve the tool use when prompted (or use auto-approval)

Example: "Flash the new firmware to the STM32 board and run the sensor self-test" might use an MCP tool that interfaces with a J-Link debug probe.
