...
description: "Learn about the two primary transport mechanisms for communication between Aeriocode and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Aeriocode and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Aeriocode) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Aeriocode
...
-   Single-client scenarios (one Aeriocode instance per server)
...
1. The client (Aeriocode) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Aeriocode instance
...
STDIO servers run locally on the same machine as Aeriocode, which has several important implications:
...
-   **Execution**: Starts and stops with Aeriocode (child process lifecycle)
...
-   Start when needed by Aeriocode
-   Not require network configuration
-   Need to be installed alongside Aeriocode or via a package manager
...
## Configuring Transports in Aeriocode

For detailed information on configuring STDIO and SSE transports in Aeriocode, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
