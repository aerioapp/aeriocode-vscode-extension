...
description: "Complete guide to resolving terminal integration issues in Aeriocode"
---

This guide helps you resolve terminal integration issues in Aeriocode. Terminal integration is crucial for Aeriocode to execute commands and read their output, enabling it to understand errors, test results, and command responses.

<Tip>
	If you're experiencing terminal issues, try switching to a simpler shell like `bash` in the Aeriocode settings, under "Terminal Settings"
...
```mermaid
graph TD
    A[Terminal Issue] --> B{Can Aeriocode execute commands?}
    B -->|No| C[Shell Integration Unavailable]
    B -->|Yes| D{Can Aeriocode see the output?}
...
-   Commands execute but Aeriocode can't read output
-   Terminal works fine manually but not with Aeriocode
...
-   Aeriocode states in chat: "[Command is running but producing no output]"
-   Commands complete but Aeriocode doesn't see results
...
-   **Increase Shell Integration Timeout**
    ...
    1. Within Aeriocode, left-click the **Settings** button in the top right-hand corner of the chat window
...
-   Recommend AI to use file output instead
    1. Tell Aeriocode in chat or Aeriocode rules, to use `command > output.txt` before reading the file/s

<Tip>
	This family of issues is only partially solved in the latest Aeriocode versions, so if you still face this, create a GitHub issue
...
-   Docker builds never complete in Aeriocode
-   Progress bars consume thousands of tokens
-   The Aeriocode button "Proceed while running" doesn't work properly in chat

<Tip>
	This family of issues has been solved in latest Aeriocode versions but if you still face any issues, then create a GitHub issue
	for this.
</Tip>

## Terminal Settings Explained

Access these in Aeriocode by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Aeriocode uses for commands
...
-   **What it does**: How long Aeriocode waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Aeriocode reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
3. Select "WSL Bash" as terminal profile in Aeriocode

#### Path Issues

Windows path problems:

1. Use forward slashes in Aeriocode: `C:/Users/...`
...
1. Install Aeriocode on the remote machine, not locally
...
1. Install Aeriocode in the container
...
3. Choose "Trace"
4. Check Output panel → "Aeriocode" for terminal logs
...
### Why does Aeriocode create so many terminals?

When shell integration fails, Aeriocode can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Aeriocode officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
Working integration shows command output in Aeriocode's chat. Failed integration shows "Shell Integration Unavailable" or "[Command is running but producing no output]".
...
2. **Report the Issue**:
    - Use `/reportbug` in Aeriocode github issues
