# Aeriocode Evaluation System

This directory contains the evaluation system for benchmarking Aeriocode against various coding evaluation frameworks.

## Overview

The Aeriocode Evaluation System allows you to:

1. Run Aeriocode against standardized coding benchmarks
...
- Node.js 16+
- VSCode with Aeriocode extension installed
- Git

### Activation Mechanism

The evaluation system uses an `evals.env` file approach to activate test mode in the Aeriocode extension. When an evaluation is run:

1. The CLI creates an `evals.env` file in the workspace directory
2. The Aeriocode extension activates due to the `workspaceContains:evals.env` activation event
...
The Aeriocode Evaluation System includes a specialized suite for evaluating how well models can make precise edits to files using the `replace_in_file` tool.
...
├── cases/                  # Test case JSON files
├── results/                # Evaluation results
├── AeriocodeWrapper.ts         # Wrapper for model interaction
