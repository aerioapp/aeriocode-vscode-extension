...
/**
 * Spawn a VSCode instance with the Aeriocode extension
...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	if (!vsixPath) {
		try {
			...
			const aeriocodeRoot = path.resolve(process.cwd(), "..", "..")
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-aeriocode-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-aeriocode-eval-ext-${Date.now()}`)
	...
	fs.writeFileSync(
		evalsEnvPath,
		`# This file activates Aeriocode test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the Aeriocode extension
...
`,
	...
	// and configure Aeriocode to auto-open on startup
	...
	const settings = {
		...
		// Auto-open Aeriocode on startup
		"aeriocode.autoOpenOnStartup": true,

		// Show the activity bar and sidebar
		"workbench.activityBar.visible": true,
		"workbench.sideBar.visible": true,
		"workbench.view.extension.aeriocode.aeriocode-ActivityBar.visible": true,
	...
	console.log(`Created settings.json to disable workspace trust and auto-open Aeriocode`)

	// Create keybindings.json to automatically open Aeriocode on startup
	const keybindingsPath = path.join(settingsDir, "keybindings.json")
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.aeriocode.aeriocode-ActivityBar",
			when: "viewContainer.workbench.view.extension.aeriocode.aeriocode-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
			command: "aeriocode.openInNewTab",
			when: "viewContainer.workbench.view.extension.aeriocode-ActivityBar.enabled",
		},
	]
	fs.writeFileSync(keybindingsPath, JSON.stringify(keybindings, null, 2))
	console.log(`Created keybindings.json to help with Aeriocode activation`)
	...
	const args = [
		...
		"--start-up-extension",
		"aeriocode.aeriocode",
		// Run a command on startup to open Aeriocode
		"--command",
		"workbench.view.extension.aeriocode.aeriocode-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			// Try to open Aeriocode in the sidebar
			require('vscode').commands.executeCommand('workbench.view.extension.aeriocode.aeriocode-ActivityBar');
			
			// Also try to open Aeriocode in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('aeriocode.openInNewTab');
			}, 5000);
		}, 5000);
	`
	fs.writeFileSync(startupScriptPath, startupScript)
	console.log(`Created startup script to activate Aeriocode`)
	...
	try {
		...
		const extensionDir = path.join(tempExtensionsDir, "aeriocode-activator")
		fs.mkdirSync(extensionDir, { recursive: true })

		// Create package.json for the extension
		const packageJsonPath = path.join(extensionDir, "package.json")
		const packageJson = {
			name: "aeriocode-activator",
			displayName: "Aeriocode Activator",
			description: "Activates Aeriocode and starts the test server",
			version: "0.0.1",
			engines: {
				vscode: "^1.60.0",
			},
			...
			contributes: {
				commands: [
					{
						command: "aeriocode-activator.activate",
						title: "Activate Aeriocode",
					},
				],
			},
		}
		...
		const extensionJs = `
			...
			function activate(context) {
				console.log('Aeriocode Activator is now active!');
				
				// Register the command to activate Aeriocode
				let disposable = vscode.commands.registerCommand('aeriocode-activator.activate', async function () {
					try {
						// Make sure the Aeriocode extension is activated
						const extension = vscode.extensions.getExtension('aeriocode.aeriocode');
						if (!extension) {
							console.error('Aeriocode extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating Aeriocode extension...');
							await extension.activate();
						}
						
						// Show the Aeriocode sidebar
						console.log('Opening Aeriocode sidebar...');
						await vscode.commands.executeCommand('workbench.view.extension.aeriocode.aeriocode-ActivityBar');
						
						// Wait a moment for the sidebar to initialize
						await new Promise(resolve => setTimeout(resolve, 2000));
						
						// Also open Aeriocode in a tab as a fallback
						console.log('Opening Aeriocode in a tab...');
						await vscode.commands.executeCommand('aeriocode.openInNewTab');
						...
						const aeriocodeRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(aeriocodeRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(aeriocodeRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating Aeriocode:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('aeriocode-activator.activate');
				}, 5000);
			}
		...
		console.log(`Created Aeriocode Activator extension`)

		// Try multiple approaches to activate the extension
		let serverStarted = false

		// Create an activation script to run in VS Code
		const activationScriptPath = path.join(settingsDir, "activate-aeriocode.js")
		const activationScript = `
			// This script will be executed to activate Aeriocode and start the test server
			const vscode = require('vscode');
			
			// Execute the aeriocode-activator.activate command
			vscode.commands.executeCommand('aeriocode-activator.activate');
		`
		...
		try {
			console.log("Executing activation script to start Aeriocode and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the Aeriocode extension in VS Code")
		}
