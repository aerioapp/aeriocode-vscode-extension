...
from utils import get_database_connection  # noqa: F401
...
def render_hero_section(current_run, model_performance):
    ...
    model_performance["avg_round_trip_ms"].mean()
...
def render_comparison_charts(model_performance):
    ...
    with col1:
        ...
        fig_first_edit = px.bar(
            ...
            labels={
                "avg_first_edit_ms": "Time to First Edit (ms)",
                "model_id": "Model",
            },
...
def render_detailed_analysis(run_id, model_id):
    ...
    st.info(
        f"Showing all {len(detailed_results)} results ({len(valid_results)} valid, {len(detailed_results) - len(valid_results)} invalid)"
    )
    ...
    with col1:
        ...
        st.metric(
            "Success Rate",
            f"{success_count}/{total_count} ({success_count / total_count:.1%} of valid results)",
        )
...
def render_result_detail(result):
    ...
    if not is_valid:
        st.warning(
            "⚠️ **This is an invalid result** - The model didn't call the replace_in_file tool or edited the wrong file. This result is excluded from success rate calculations."
        )
...
def render_file_and_edits_view(result):
    ...
    with col1:
        ...
        if has_original:
            ...
            with st.expander("View Original File Content", expanded=True):
                ...
                unique_suffix = (
                    str(result.name if hasattr(result, "name") else result["task_id"])
                    .replace("-", "_")
                    .replace(".", "_")
                )
    ...
    with col2:
        ...
        if not result["succeeded"]:
            ...
            if not pd.isna(result["error_enum"]):
                error_description = get_error_description(result["error_enum"], result.get("error_string"))
            ...
            # For valid results that failed, check for diff application failures
                ...
                if "does not match anything in the file" in str(raw_output).lower():
                    ...
                    st.info(
                        "💡 The SEARCH block in the diff didn't match any content in the original file. This usually means the model hallucinated code that doesn't exist."
                    )
                    ...
                    if error_lines:
                        ...
                        st.warning("⚠️ **Diff Application Failed**")
                        st.info(
                            "💡 The diff couldn't be applied to the original file. Check the raw output and parsed tool call for more details."
                        )
                else:
                    # Generic diff application failure
                    st.warning("⚠️ **Diff Application Failed**")
                    st.info(
                        "💡 The model made a valid tool call but the diff couldn't be applied to the original file. This usually indicates a mismatch between the expected and actual file content."
                    )
        else:
