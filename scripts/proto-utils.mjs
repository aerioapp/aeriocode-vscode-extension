...
// Get the fully qualified name for a proto type, e.g. getFqn('StringRequest') returns 'aeriocode.StringRequest'
...
export async function loadServicesFromProtoDescriptor() {
	...
	for (const [name, def] of Object.entries(proto.aeriocode)) {
		if (def && "service" in def) {
			protobusServices[name] = def
		} else {
			addTypeNameToFqn(name, `proto.aeriocode.${name}`)
		}
	}
