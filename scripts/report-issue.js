...
const getAeriocodeVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const aeriocodeMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/)
		return aeriocodeMatch ? aeriocodeMatch[1] : "Not installed"
	} catch (err) {
...
const collectSystemInfo = () => {
	...
	return {
		...
		aeriocodeVersion: getAeriocodeVersion(),
	}
}
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/aeriocode/aeriocode/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&aeriocode-version=${systemInfo.aeriocodeVersion}` +
		`&system-info=${
