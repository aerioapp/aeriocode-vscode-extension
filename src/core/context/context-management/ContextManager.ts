...
import { AeriocodeApiReqInfo, AeriocodeMessage } from "@shared/ExtensionMessage"
...
export class ContextManager {
	...
	async getNewContextMessagesAndMetadata(
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = aeriocodeMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const timestamp = previousRequest.ts
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: AeriocodeApiReqInfo = JSON.parse(previousRequest.text)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout aeriocode)
