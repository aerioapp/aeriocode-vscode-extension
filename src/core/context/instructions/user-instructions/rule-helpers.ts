...
export async function synchronizeRuleToggles(
	...
	currentToggles: AeriocodeRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
 ...
 */
export function combineRuleToggles(toggles1: AeriocodeRulesToggles, toggles2: AeriocodeRulesToggles): AeriocodeRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: AeriocodeRulesToggles) => {
...
/**
 * Handles converting any directory into a file (specifically used for .aeriocoderules and .aeriocoderules/workflows)
 * The old .aeriocoderules file or .aeriocoderules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalAeriocodeDirExists(aeriocoderulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(aeriocoderulePath)

		if (exists && !(await isDirectory(aeriocoderulePath))) {
			// logic to convert .aeriocoderules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(aeriocoderulePath, "utf8")
			const tempPath = aeriocoderulePath + ".bak"
			await fs.rename(aeriocoderulePath, tempPath) // create backup
			try {
				await fs.mkdir(aeriocoderulePath, { recursive: true })
				await fs.writeFile(path.join(aeriocoderulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(aeriocoderulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, aeriocoderulePath) // restore backup
				} catch (restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalAeriocodeWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalAeriocodeWorkflowFilePath, filename)
			} else {
				const globalAeriocodeRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalAeriocodeRulesFilePath, filename)
			}
		} else {
			const localAeriocodeRulesFilePath = path.resolve(cwd, GlobalFileNames.aeriocodeRules)

			const hasError = await ensureLocalAeriocodeDirExists(localAeriocodeRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localAeriocodeRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalAeriocodeDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// aeriocoderules file creation
				filePath = path.join(localAeriocodeRulesFilePath, filename)
			}
		}
...
export async function deleteRuleFile(
	...
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const toggles = ((await getGlobalState(context, "globalWorkflowToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateGlobalState(context, "globalWorkflowToggles", toggles)
			} else {
				const toggles = ((await getGlobalState(context, "globalAeriocodeRulesToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateGlobalState(context, "globalAeriocodeRulesToggles", toggles)
			}
		} else {
			if (type === "workflow") {
				const toggles = ((await getWorkspaceState(context, "workflowToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateWorkspaceState(context, "workflowToggles", toggles)
			} else if (type === "cursor") {
				const toggles = ((await getWorkspaceState(context, "localCursorRulesToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateWorkspaceState(context, "localCursorRulesToggles", toggles)
			} else if (type === "windsurf") {
				const toggles = ((await getWorkspaceState(context, "localWindsurfRulesToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateWorkspaceState(context, "localWindsurfRulesToggles", toggles)
			} else {
				const toggles = ((await getWorkspaceState(context, "localAeriocodeRulesToggles")) as AeriocodeRulesToggles) || {}
				delete toggles[rulePath]
				await updateWorkspaceState(context, "localAeriocodeRulesToggles", toggles)
			}
		}
