import type { ToggleCursorRuleRequest } from "@shared/proto/aeriocode/file"
 ...
 */
export async function toggleCursorRule(controller: Controller, request: ToggleCursorRuleRequest): Promise<AeriocodeRulesToggles> {
	...
	const toggles = ((await getWorkspaceState(controller.context, "localCursorRulesToggles")) as AppAeriocodeRulesToggles) || {}
	toggles[rulePath] = enabled
	await updateWorkspaceState(controller.context, "localCursorRulesToggles", toggles)

	// Get the current state to return in the response
	const cursorToggles =
		((await getWorkspaceState(controller.context, "localCursorRulesToggles")) as AppAeriocodeRulesToggles) || {}

	return AeriocodeRulesToggles.create({
