...
import { PostHogClientProvider, telemetryService } from "@/services/telemetry"
...
export class Controller {
	...
	mcpHub: McpHub
	accountService: AeriocodeAccountService
	...
	constructor(
		...
		HostProvider.get().logToChannel("AeriocodeProvider instantiated")
		this.accountService = AeriocodeAccountService.getInstance()
	...
	*/
	...
	async handleSignOut() {
		try {
			// TODO: update to aeriocodeAccountId and then move aeriocodeApiKey to a clear function.
			this.cacheService.setSecret("aeriocodeAccountId", undefined)
			await updateGlobalState(this.context, "userInfo", undefined)

			// Update API providers through cache service to Aeriocode-only
			const apiConfiguration = this.cacheService.getApiConfiguration()
			const updatedConfig = {
				...apiConfiguration,
				planModeApiProvider: "aeriocode" as ApiProvider,
				actModeApiProvider: "aeriocode" as ApiProvider,
			}
			...
			HostProvider.window.showMessage({
				...
				message: "Successfully logged out of Aeriocode",
			})
		} catch (error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async initTask(task?: string, images?: string[], files?: string[], historyItem?: HistoryItem) {
		...
		// Ensure API configuration has the correct model IDs based on current mode
		// This fixes the bug where new chats would jump to default model instead of using selected model
		const effectiveApiConfiguration = { ...apiConfiguration }

		// Get the current model ID from the UI selection, not from hardcoded defaults
		// The UI always has a model selected, so we should use the current mode's model ID
		// Always use the latest model selection from the cache service to ensure we use the UI-selected model
		const currentApiConfig = this.cacheService.getApiConfiguration()

		if (mode === "plan") {
			// Always use the current plan mode model ID from cache, which reflects the UI selection
			effectiveApiConfiguration.planModeApiModelId =
				currentApiConfig.planModeApiModelId || currentApiConfig.actModeApiModelId
		} else {
			// Always use the current act mode model ID from cache, which reflects the UI selection
			effectiveApiConfiguration.actModeApiModelId =
				currentApiConfig.actModeApiModelId || currentApiConfig.planModeApiModelId
		}
	...
	async cancelTask() {
		if (this.task) {
			...
			if (this.task) {
				// 'abandoned' will prevent this aeriocode instance from affecting future aeriocode instance gui. this may happen if its hanging on a streaming request
				this.task.taskState.abandoned = true
			}
			await this.initTask(undefined, undefined, undefined, historyItem) // clears task again, so we need to abortTask manually above
			// Dont send the state to the webview, the new Aeriocode instance will send state when it's ready.
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			const aeriocodeProvider: ApiProvider = "aeriocode"
			...
			if (planActSeparateModelsSetting) {
				...
				if (currentMode === "plan") {
					updatedConfig.planModeApiProvider = aeriocodeProvider
				} else {
					updatedConfig.actModeApiProvider = aeriocodeProvider
				}
			} else {
			...
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: "Failed to log in to Aeriocode",
			})
	...
	private async fetchMcpMarketplaceFromApi(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get(`${aeriocodeEnvConfig.mcpBaseUrl}/marketplace`, {
				headers: {
	...
	private async fetchMcpMarketplaceFromApiRPC(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get(`${aeriocodeEnvConfig.mcpBaseUrl}/marketplace`, {
				headers: {
					"Content-Type": "application/json",
					"User-Agent": "aeriocode-vscode-extension",
				},
			})
	...
	// 'Add to Aeriocode' context menu in editor and code action
	...
	// 'Add to Aeriocode' context menu in Terminal
	async addSelectedTerminalOutputToChat(output: string, terminalName: string) {
		// Ensure the sidebar view is visible
		await vscode.commands.executeCommand("aeriocode.SidebarProvider.focus")
	...
	// 'Fix with Aeriocode' in code actions
	async fixWithAeriocode(code: string, filePath: string, languageId: string, diagnostics: vscode.Diagnostic[]) {
		// Ensure the sidebar view is visible
		await vscode.commands.executeCommand("aeriocode.SidebarProvider.focus")
		await setTimeoutPromise(100)
		...
		console.log("fixWithAeriocode", code, filePath, languageId, diagnostics, problemsString)
	}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const aeriocodeMessages = this.task?.messageStateHandler.getAeriocodeMessages() || []
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of aeriocode messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between AeriocodeProvider instances since there could be multiple instances of the extension running at once. For example when we cached aeriocode messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each AeriocodeProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
