...
import { McpDownloadResponse } from "@shared/proto/aeriocode/mcp"
...
export async function downloadMcp(controller: Controller, request: StringRequest): Promise<McpDownloadResponse> {
	try {
		...
		const response = await axios.post<McpDownloadResponse>(
			`${aeriocodeEnvConfig.mcpBaseUrl}/download`,
			{ mcpId },
			{
...
- Use "${mcpDetails.mcpId}" as the server name in aeriocode_mcp_settings.json.
- Create the directory for the new MCP server before starting installation.
- Make sure you read the user's existing aeriocode_mcp_settings.json file before editing it with this new mcp, to not overwrite any existing servers.
