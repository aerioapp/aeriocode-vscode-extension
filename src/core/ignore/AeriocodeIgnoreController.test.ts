import { AeriocodeIgnoreController } from "./AeriocodeIgnoreController"
...
describe("AeriocodeIgnoreController", () => {
	let tempDir: string
	let controller: AeriocodeIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .aeriocodeignore file
		await fs.writeFile(
			path.join(tempDir, ".aeriocodeignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new AeriocodeIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .aeriocodeignore file", async () => {
			const result = controller.validateAccess(".aeriocodeignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .aeriocodeignore", async () => {
			// Create a new .aeriocodeignore with comments
			await fs.writeFile(
				path.join(tempDir, ".aeriocodeignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .aeriocodeignore gracefully", async () => {
			// Create a new controller in a directory without .aeriocodeignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new AeriocodeIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .aeriocodeignore", async () => {
			await fs.writeFile(path.join(tempDir, ".aeriocodeignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .aeriocodeignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".aeriocodeignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .aeriocodeignore
			controller = new AeriocodeIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .aeriocodeignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .aeriocodeignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".aeriocodeignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".aeriocodeignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
