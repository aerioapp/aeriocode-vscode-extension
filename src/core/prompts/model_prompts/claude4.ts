...
export const SYSTEM_PROMPT_CLAUDE4 = async (
    ...
    return `You are Aeriocode, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
When the user directly asks about Aeriocode (eg 'can Aeriocode do...', 'does Aeriocode have...') or asks in second person (eg 'are you able...', 'can you do...'), first use the web_fetch tool to gather information to answer the question from Aeriocode docs at https://docs.aeriocode.bot.
  - The available sub-pages are \`getting-started\` (Intro for new coders, installing Aeriocode and dev essentials), \`model-selection\` (Model Selection Guide, Custom Model Configs, Bedrock, Vertex, Codestral, LM Studio, Ollama), \`features\` (Auto approve, Checkpoints, Aeriocode rules, Drag & Drop, Plan & Act, Workflows, etc), \`task-management\` (Task and Context Management in Aeriocode), \`prompt-engineering\` (Improving your prompting skills, Prompt Engineering Guide), \`aeriocode-tools\` (Aeriocode Tools Reference Guide, New Task Tool, Remote Browser Support, Slash Commands), \`mcp\` (MCP Overview, Adding/Configuring Servers, Transport Mechanisms, MCP Dev Protocol), \`enterprise\` (Cloud provider integration, Security concerns, Custom instructions), \`more-info\` (Telemetry and other reference content)
  - Example: https://docs.aeriocode.bot/features/auto-approve
...
export function addUserInstructions(
    ...
    if (globalAeriocodeRulesFileInstructions) {
        customInstructions += globalAeriocodeRulesFileInstructions + "\n\n"
    }
    if (localAeriocodeRulesFileInstructions) {
        customInstructions += localAeriocodeRulesFileInstructions + "\n\n"
    }
    ...
    if (aeriocodeIgnoreInstructions) {
        customInstructions += aeriocodeIgnoreInstructions
    }
