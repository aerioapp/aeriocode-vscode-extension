...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsAeriocoderulesFileCheck: boolean }> {
	...
	for (const { tag, regex } of tagPatterns) {
		...
		if (match) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsAeriocoderulesFileCheck: commandName === "newrule" ? true : false }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsAeriocoderulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsAeriocoderulesFileCheck: false }
}
