...
export const GlobalFileNames = {
	...
	mcpSettings: "aeriocode_mcp_settings.json",
	aeriocodeRules: ".aeriocoderules",
	workflows: ".aeriocoderules/workflows",
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const aeriocodeRulesDir = path.join(userDocumentsPath, "Aeriocode", "Rules")
	try {
		await fs.mkdir(aeriocodeRulesDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "Aeriocode", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return aeriocodeRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const aeriocodeWorkflowsDir = path.join(userDocumentsPath, "Aeriocode", "Workflows")
	try {
		await fs.mkdir(aeriocodeWorkflowsDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "Aeriocode", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return aeriocodeWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Aeriocode", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (error) {
		return "~/Documents/Aeriocode/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function getSavedAeriocodeMessages(context: vscode.ExtensionContext, taskId: string): Promise<AeriocodeMessage[]> {
	...
	if (await fileExistsAtPath(filePath)) {
		...
		if (await fileExistsAtPath(oldPath)) {
			try {
				...
				console.error(`Failed to parse JSON from old path ${oldPath}:`, error)
				// Return empty array if JSON parsing fails
				return []
			}
		}
...
export async function saveAeriocodeMessages(context: vscode.ExtensionContext, taskId: string, uiMessages: AeriocodeMessage[]) {
	try {
