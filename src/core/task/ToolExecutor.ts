...
import {
	...
	browserActions,
	AeriocodeAsk,
	AeriocodeAskQuestion,
	AeriocodeAskUseMcpServer,
	AeriocodePlanModeResponse,
	AeriocodeSay,
	AeriocodeSayBrowserAction,
	AeriocodeSayTool,
...
export class ToolExecutor {
	...
	constructor(
		...
		private removeLastPartialMessageIfExistsWithType: (
			type: "ask" | "say",
			askOrSay: AeriocodeAsk | AeriocodeSay,
		) => Promise<void>,
	...
	private askApproval = async (type: AeriocodeAsk, block: ToolUse, partialMessage: string) => {
	...
	public async executeTool(block: ToolUse): Promise<void> {
		...
		switch (block.name) {
			...
			case "replace_in_file": {
				...
				if (!accessAllowed) {
					await this.say("aeriocodeignore_error", relPath)
					this.pushToolResult(formatResponse.toolError(formatResponse.aeriocodeIgnoreError(relPath)), block)
					await this.saveCheckpoint()
					break
				}
				...
				try {
					...
					if (diff) {
						...
						// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Aeriocode throws error
					...
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, relPath)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
						...
						// Mark the file as edited by Aeriocode to prevent false "recently modified" warnings
						this.fileContextTracker.markFileAsEditedByAeriocode(relPath)
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to read ${path.basename(absolutePath)}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to view directory ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
			...
			case "list_code_definition_names": {
				...
				try {
					if (block.partial) {
						...
						const absolutePath = path.resolve(this.cwd, relDirPath)
						const result = await parseSourceCodeForDefinitionsTopLevel(absolutePath, this.aeriocodeIgnoreController)
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to view source code definitions in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to search files in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
					...
					if (block.partial) {
						if (action === "launch") {
							...
							await this.say(
								...
								} satisfies AeriocodeSayBrowserAction),
								undefined,
								undefined,
								block.partial,
							)
						...
						if (action === "launch") {
							...
							if (this.shouldAutoApproveTool(block.name)) {
								...
								showNotificationForApprovalIfAutoApprovalEnabled(
									`Aeriocode wants to use a browser and launch ${url}`,
									this.autoApprovalSettings.enabled,
									this.autoApprovalSettings.enableNotifications,
								)
								this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
			...
			case "execute_command": {
				...
				try {
					if (block.partial) {
						...
						const ignoredFileAttemptedToAccess = this.aeriocodeIgnoreController.validateCommand(command)
						if (ignoredFileAttemptedToAccess) {
							await this.say("aeriocodeignore_error", ignoredFileAttemptedToAccess)
							this.pushToolResult(
								formatResponse.toolError(formatResponse.aeriocodeIgnoreError(ignoredFileAttemptedToAccess)),
								block,
							)
							await this.saveCheckpoint()
							break
						...
						if (
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to execute a command: ${command}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
					...
					if (block.partial) {
						...
						if (mcp_arguments) {
							try {
								...
								await this.say(
									"error",
									`Aeriocode tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
								)
								this.pushToolResult(
						...
						if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to use ${tool_name} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool(block.name)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to access ${uri} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
			...
			case "ask_followup_question": {
				...
				} satisfies AeriocodeAskQuestion
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Aeriocode has a question...",
			...
			case "new_task": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Aeriocode wants to start a new task...",
								message: `Aeriocode is suggesting to start a new task with: ${context}`,
							})
						}
			...
			case "condense": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Aeriocode wants to condense the conversation...",
								message: `Aeriocode is suggesting to condense your conversation with: ${context}`,
							})
						}
			...
			case "report_bug": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Aeriocode wants to create a github issue...",
								message: `Aeriocode is suggesting to create a github issue with the title: ${title}`,
							})
						}

						// Derive system information values algorithmically
						const operatingSystem = os.platform() + " " + os.release()
						const aeriocodeVersion =
							vscode.extensions.getExtension(getExtensionId())?.packageJSON.version || "Unknown"
						...
						const providerAndModel = this.api.getModel().id
						...
						const bugReportData = JSON.stringify({
							...
							aeriocode_version: aeriocodeVersion,
						})
						...
						if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
							...
							try {
								...
								params.set("aeriocode-version", aeriocodeVersion)
								...
								await createAndOpenGitHubIssue("aeriocode", "aeriocode", "bug_report.yml", params)
							} catch (error) {
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool("web_fetch" as ToolUseName)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Aeriocode wants to fetch content from ${url}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
			...
			case "plan_mode_respond": {
				...
				const sharedMessage = {
					...
					options: parsePartialArrayString(this.removeClosingTag(block, "options", optionsRaw)),
				} satisfies AeriocodePlanModeResponse
				try {
					if (block.partial) {
						...
						// if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
						// 	showSystemNotification({
						// 		subtitle: "Aeriocode has a response...",
			...
			case "attempt_completion": {
				...
				const addNewChangesFlagToLastCompletionResultMessage = async () => {
					...
					const lastCompletionResultMessage =
						lastCompletionResultMessageIndex !== -1 ? aeriocodeMessages[lastCompletionResultMessageIndex] : undefined
					if (
						...
						await this.messageStateHandler.updateAeriocodeMessage(lastCompletionResultMessageIndex, {
				...
				try {
					...
					if (block.partial) {
						if (command) {
							...
							// const secondLastMessage = this.aeriocodeMessages.at(-2)
						...
						if (command) {
							...
							// Ask first to set aeriocodeAsk in webview before blocking checkpoint operations.
							// This prevents a race condition where the user sends a message during
							// saveCheckpoint() and it gets silently dropped because aeriocodeAsk is undefined.
							const {
								response,
								text,
								images,
