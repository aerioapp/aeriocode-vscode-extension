...
import { AeriocodeAskResponse, AeriocodeCheckpointRestore } from "@shared/WebviewMessage"
...
import { AeriocodeErrorType } from "@services/error/AeriocodeError"
...
import {
	getGlobalAeriocodeRules,
	getLocalAeriocodeRules,
	refreshAeriocodeRulesToggles,
} from "@core/context/instructions/user-instructions/aeriocode-rules"
import {
	getLocalCursorRules,
...
import { convertAeriocodeMessageToProto } from "@shared/proto-conversions/aeriocode-message"
...
import { ensureLocalAeriocodeDirExists } from "../context/instructions/user-instructions/rule-helpers"
...
export class Task {
	...
	private aeriocodeIgnoreController: AeriocodeIgnoreController
	...
	constructor(
		...
		this.aeriocodeIgnoreController = new AeriocodeIgnoreController(cwd)
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
				const lastApiReqStartedIndex = findLastIndex(aeriocodeMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: AeriocodeApiReqInfo = JSON.parse(
							aeriocodeMessages[lastApiReqStartedIndex].text || "{}",
						)
						...
						await this.messageStateHandler.updateAeriocodeMessage(lastApiReqStartedIndex, {
		...
		// Aeriocode provider doesn't use reasoning effort settings

		// Now that taskId is initialized, we can build the API handler
		// Pass context information for backend system prompt generation
		const contextInfo = {
			cwd: this.cwd,
			osName: process.platform,
			shell: process.env.SHELL || (process.platform === "win32" ? "cmd" : "bash"),
			homeDir: require("os").homedir(),
			browserWidth: this.browserSettings.viewport?.width || 1280,
			browserHeight: this.browserSettings.viewport?.height || 720,
			mcpServers: this.mcpHub.getServers().map((server) => ({
				name: server.name,
				tools: server.tools,
		...
		if (historyItem) {
			...
			telemetryService.captureTaskRestarted(this.taskId, this.ulid, "aeriocode")
		} else {
			// New task started
			telemetryService.captureTaskCreated(this.taskId, this.ulid, "aeriocode")
		}
		...
		this.toolExecutor = new ToolExecutor(
			...
			this.aeriocodeIgnoreController,
	...
	async restoreCheckpoint(messageTs: number, restoreType: AeriocodeCheckpointRestore, offset?: number) {
		const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
		const messageIndex = aeriocodeMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
		// Find the last message before messageIndex that has a lastCheckpointHash
		const lastHashIndex = findLastIndex(aeriocodeMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
		const message = aeriocodeMessages[messageIndex]
		const lastMessageWithHash = aeriocodeMessages[lastHashIndex]

		if (!message) {
			console.error("Message not found", aeriocodeMessages)
			return
		}
		...
		if (!didWorkspaceRestoreFail) {
			switch (restoreType) {
				...
				case "taskAndWorkspace": {
					...
					const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
					const deletedMessages = aeriocodeMessages.slice(messageIndex + 1)
					...
					const newAeriocodeMessages = aeriocodeMessages.slice(0, messageIndex + 1)
					await this.messageStateHandler.overwriteAeriocodeMessages(newAeriocodeMessages) // calls saveAeriocodeMessages which saves historyItem
			...
			await this.messageStateHandler.saveAeriocodeMessagesAndUpdateHistory()
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean) {
		...
		const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
		const messageIndex = aeriocodeMessages.findIndex((m) => m.ts === messageTs)
		const message = aeriocodeMessages[messageIndex]
		if (!message) {
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.messageStateHandler.getAeriocodeMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges() {
		...
		const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
		const messageIndex = findLastIndex(aeriocodeMessages, (m) => m.say === "completion_result")
		const message = aeriocodeMessages[messageIndex]
		if (!message) {
	...
	async ask(
		...
		response: AeriocodeAskResponse
		text?: string
		images?: string[]
		files?: string[]
	}> {
		// If this Aeriocode instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of Aeriocode now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set Aeriocode = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.taskState.abort) {
			throw new Error("Aeriocode instance aborted")
		}
		let askTs: number
		if (partial !== undefined) {
			const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
			const lastMessage = aeriocodeMessages.at(-1)
			const lastMessageIndex = aeriocodeMessages.length - 1
			const isUpdatingPreviousPartial =
				lastMessage && lastMessage.partial && lastMessage.type === "ask" && lastMessage.ask === type
			if (partial) {
				if (isUpdatingPreviousPartial) {
					// existing partial message, so update it
					await this.messageStateHandler.updateAeriocodeMessage(lastMessageIndex, {
						text,
						partial,
					})
					// todo be more efficient about saving and posting only new data or one whole message at a time so ignore partial for saves, and only post parts of partial message instead of whole array in new listener
					// await this.saveAeriocodeMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertAeriocodeMessageToProto(lastMessage)
					...
					await this.messageStateHandler.addToAeriocodeMessages({
						ts: askTs,
						type: "ask",
						ask: type,
						text,
			...
			// const lastMessage = this.aeriocodeMessages.at(-1)
			this.taskState.askResponse = undefined
			this.taskState.askResponseText = undefined
			this.taskState.askResponseImages = undefined
			this.taskState.askResponseFiles = undefined
	...
	async handleWebviewAskResponse(askResponse: AeriocodeAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(type: AeriocodeSay, text?: string, images?: string[], files?: string[], partial?: boolean): Promise<undefined> {
		if (this.taskState.abort) {
			throw new Error("Aeriocode instance aborted")
		}

		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getAeriocodeMessages().at(-1)
			const isUpdatingPreviousPartial =
	...
	async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Aeriocode tried to use ${toolName}${
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: AeriocodeAsk | AeriocodeSay) {
		...
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setAeriocodeMessages(aeriocodeMessages.slice(0, -1))
			await this.messageStateHandler.saveAeriocodeMessagesAndUpdateHistory()
		}
	}
	...
	private async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.aeriocodeIgnoreController.initialize()
		} catch (error) {
			console.error("Failed to initialize AeriocodeIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and aeriocodeMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the aeriocodeMessages might not be empty, so we need to set it to [] when we create a new Aeriocode client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setAeriocodeMessages([])
	...
	private async resumeTaskFromHistory() {
		...
		const savedAeriocodeMessages = await getSavedAeriocodeMessages(this.getContext(), this.taskId)

		// Remove any resume messages that may have been added before
		const lastRelevantMessageIndex = findLastIndex(
			savedAeriocodeMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			savedAeriocodeMessages.splice(lastRelevantMessageIndex + 1)
		}
		...
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedAeriocodeMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: AeriocodeApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savedAeriocodeMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwriteAeriocodeMessages(savedAeriocodeMessages)
		this.messageStateHandler.setAeriocodeMessages(await getSavedAeriocodeMessages(this.getContext(), this.taskId))

		// Now present the aeriocode messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastAeriocodeMessage = this.messageStateHandler
			.getAeriocodeMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: AeriocodeAsk
		if (lastAeriocodeMessage?.ask === "completion_result") {
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with aeriocode messages
		...
		const agoText = (() => {
			const timestamp = lastAeriocodeMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastAeriocodeMessage?.ts && Date.now() - lastAeriocodeMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeAeriocodeRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that aeriocode will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but Aeriocode is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Aeriocode responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async abortTask() {
		...
		this.aeriocodeIgnoreController.dispose()
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
		...
		this.messageStateHandler.getAeriocodeMessages().forEach((message) => {
		...
		if (!isAttemptCompletionMessage) {
			...
			// Create a checkpoint commit and update aeriocodeMessages with a commitHash
				...
				this.checkpointTracker.commit().then(async (commitHash) => {
					if (commitHash) {
						const lastCheckpointMessageIndex = findLastIndex(
							this.messageStateHandler.getAeriocodeMessages(),
							(m) => m.say === "checkpoint_created",
						)
						if (lastCheckpointMessageIndex !== -1) {
							await this.messageStateHandler.updateAeriocodeMessage(lastCheckpointMessageIndex, {
		...
		// for (let i = this.aeriocodeMessages.length - 1; i >= 0; i--) {
		// 	const message = this.aeriocodeMessages[i]
	...
	private async executeCommandInNode(command: string): Promise<[boolean, ToolResponse]> {
		try {
			...
			// Create a timeout promise that rejects after 600 seconds (10 minutes)
			const timeoutPromise = new Promise<never>((_, reject) => {
				setTimeout(() => {
					if (childProcess.pid) {
						childProcess.kill("SIGKILL") // Use SIGKILL for more forceful termination
					}
					reject(new Error("Command timeout after 600s"))
				}, 600000)
			})
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		// Wait for MCP servers to be connected before making API request
		await pWaitFor(() => this.mcpHub.isConnecting !== true, {
			timeout: 30_000, // Increased from 10s to 30s for slow MCP servers
		}).catch(() => {
		...
		// aeriocode browser tool uses image recognition for navigation (requires model image support).
		const modelSupportsBrowserUse = modelInfo.info.supportsImages ?? false

		const supportsBrowserUse = modelSupportsBrowserUse && !disableBrowserTool // only enable browser use if the model supports it and the user hasn't disabled it

		// Collect user instructions locally in the extension
		...
		const { globalToggles, localToggles } = await refreshAeriocodeRulesToggles(this.getContext(), this.cwd)
		...
		const globalAeriocodeRulesFileInstructions = await getGlobalAeriocodeRules(globalAeriocodeRulesFilePath, globalToggles)

		const localAeriocodeRulesFileInstructions = await getLocalAeriocodeRules(this.cwd, localToggles)
		...
		const aeriocodeIgnoreContent = this.aeriocodeIgnoreController.aeriocodeIgnoreContent
		let aeriocodeIgnoreInstructions: string | undefined
		if (aeriocodeIgnoreContent) {
			aeriocodeIgnoreInstructions = formatResponse.aeriocodeIgnoreInstructions(aeriocodeIgnoreContent)
		}
		...
		if (this.api instanceof AeriocodeHandler) {
			;(this.api as any).options.userInstructions = userInstructions
		}
		...
		const stream = this.api.createMessage("", contextManagementMetadata.truncatedConversationHistory)
		...
		try {
			...
			const isAeriocode = this.api instanceof AeriocodeHandler
			const { modelId, providerId } = await this.getCurrentProviderInfo()
			const aeriocodeError = errorService.toAeriocodeError(error, modelId, providerId)

			// Capture provider failure telemetry using aeriocodeError
			// TODO: Move into errorService
			errorService.logMessage(aeriocodeError.message)
			errorService.logException(aeriocodeError)

			if (!this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				const streamingFailedMessage = aeriocodeError.serialize()
				...
				if (manualRetryApiReqIndex !== -1) {
					const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
					const currentApiReqInfo: AeriocodeApiReqInfo = JSON.parse(
						aeriocodeMessages[manualRetryApiReqIndex].text || "{}",
					)
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateAeriocodeMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
		...
		if (!block.partial || this.taskState.didRejectTool || this.taskState.didAlreadyUseTool) {
			...
			if (this.taskState.currentStreamingContentIndex < this.taskState.assistantMessageContent.length) {
				...
				try {
					...
					// Continue execution even if presentation fails
				}
				return
			}
		}
	...
	async recursivelyMakeAeriocodeRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		if (this.taskState.abort) {
			throw new Error("Aeriocode instance aborted")
		}
		...
		if (this.taskState.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Aeriocode is having trouble. Would you like to continue the task?",
				})
			}

			// Get current provider info for better error messaging
			const { modelId, providerId } = await this.getCurrentProviderInfo()
			const isClaudeModel = modelId.includes("claude")
			const isAeriocodeProvider = providerId.includes("aeriocode") || this.api instanceof AeriocodeHandler

			let errorMessage: string
			if (isClaudeModel) {
				errorMessage = `This may indicate a failure in his thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
			} else if (isAeriocodeProvider) {
				errorMessage = `Aeriocode is having difficulty completing the task. This could be due to network issues, backend service problems, or complex task requirements. Please check your connection and try simplifying the task.`
			} else {
				errorMessage = `Aeriocode uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4 Sonnet for its advanced agentic coding capabilities.`
			}

			const { response, text, images, files } = await this.ask("mistake_limit_reached", errorMessage)
			if (response === "messageResponse") {
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `Aeriocode has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			const { response, text, images, files } = await this.ask(
				"auto_approval_max_req_reached",
				`Aeriocode has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
		...
		const isFirstRequest =
			this.messageStateHandler.getAeriocodeMessages().filter((m) => m.say === "api_req_started").length === 0
			...
			try {
				...
				checkpointsWarningTimer = setTimeout(async () => {
					if (!checkpointsWarningShown) {
						...
						this.taskState.checkpointTrackerErrorMessage =
							"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Aeriocode in a project that uses git, or disabling checkpoints."
						await this.postStateToWebview()
					}
				}, 7_000)
				...
				this.checkpointTracker = await pTimeout(
					...
					{
						milliseconds: 60_000, // Increased from 15s to 60s for large repositories
						message:
							"Checkpoints taking too long to initialize. Consider re-opening Aeriocode in a project that uses git, or disabling checkpoints.",
					},
				)
				...
				if (errorMessage.includes("Checkpoints taking too long to initialize")) {
					this.taskState.checkpointTrackerErrorMessage =
						"Checkpoints initialization timed out. Consider re-opening Aeriocode in a project that uses git, or disabling checkpoints."
					await this.postStateToWebview()
				} else {
					this.taskState.checkpointTrackerErrorMessage = errorMessage // will be displayed right away since we saveAeriocodeMessages next which posts state to webview
				}
			}
		}
		...
		if (isFirstRequest && this.enableCheckpoints && this.checkpointTracker) {
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				// saveAeriocodeMessagesAndUpdateHistory will be called later after API response,
		...
		const [parsedUserContent, environmentDetails, aeriocoderulesError] = await this.loadContext(
			userContent,
			includeFileDetails,
		)

		// error handling if the user uses the /newrule command & their .aeriocoderules is a file, for file read operations didnt work properly
		if (aeriocoderulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.aeriocoderules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getAeriocodeMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateAeriocodeMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies AeriocodeApiReqInfo),
		})
		...
		try {
			...
			const abortStream = async (cancelReason: AeriocodeApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of aeriocode)
				...
				// abandoned happens when extension is no longer waiting for the aeriocode instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					this.abortTask() // if the stream failed, there's various states the task could be in (i.e. could have streamed some tools the user may have executed), so we just resort to replicating a cancel task
					const aeriocodeError = errorService.toAeriocodeError(error, this.api.getModel().id)
					const errorMessage = aeriocodeError.serialize()
			...
			// OpenRouter/Aeriocode may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantMessage.length > 0) {
				...
				const recDidEndLoop = await this.recursivelyMakeAeriocodeRequests(this.taskState.userMessageContent)
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
		// Track if we need to check aeriocoderulesFile
		let needsAeriocoderulesFileCheck = false
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							const { processedText, needsAeriocoderulesFileCheck: needsCheck } = await parseSlashCommands(
								parsedText,
							...
							if (needsCheck) {
								needsAeriocoderulesFileCheck = true
							}
		...
		// After processing content, check aeriocoderulesData if needed
		let aeriocoderulesError = false
		if (needsAeriocoderulesFileCheck) {
			aeriocoderulesError = await ensureLocalAeriocodeDirExists(this.cwd, GlobalFileNames.aeriocodeRules)
		}

		// Return all results
		return [processedUserContent, environmentDetails, aeriocoderulesError]
	}

	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		let details = ""

		// It could be useful for aeriocode to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through aeriocodeIgnoreController
		const allowedVisibleFiles = this.aeriocodeIgnoreController
		...
		// Filter paths through aeriocodeIgnoreController
		const allowedOpenTabs = this.aeriocodeIgnoreController
		...
		if (busyTerminals.length > 0) {
			...
			await pWaitFor(() => busyTerminals.every((t) => !this.terminalManager.isProcessHot(t.id)), {
				interval: 100,
				timeout: 60_000, // Increased from 15s to 60s for long-running commands
			}).catch(() => {})
		}
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if aeriocode ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
		...
		*/
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.aeriocodeIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: AeriocodeMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const aeriocodeMessages = this.messageStateHandler.getAeriocodeMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(aeriocodeMessages.slice(1)))
