...
import { AeriocodeApiReqCancelReason, AeriocodeApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const aeriocodeMessages = params.messageStateHandler.getAeriocodeMessages()
	const currentApiReqInfo: AeriocodeApiReqInfo = JSON.parse(aeriocodeMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateAeriocodeMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
