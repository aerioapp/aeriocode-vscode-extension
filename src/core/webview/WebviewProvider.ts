...
export abstract class WebviewProvider {
	public static readonly sideBarId = "aeriocode.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "aeriocode.TabPanelProvider"
	...
	public static getActiveInstance(): WebviewProvider | undefined {
		return Array.from(WebviewProvider.activeInstances).find((instance) => {
			...
			if (webview && webview.viewType === "aeriocode.TabPanelProvider" && "active" in webview) {
		...
		return /*html*/ `
			...
			<html lang="en">
				...
				<meta http-equiv="Content-Security-Policy" content="default-src 'none';
					connect-src https://*.posthog.com https://*.aerio.bot https://*.firebaseauth.com https://*.firebaseio.com https://*.googleapis.com https://*.firebase.com; 
					font-src ${this.getCspSource()} data:; 
					style-src ${this.getCspSource()} 'unsafe-inline'; 
					img-src ${this.getCspSource()} https: data:; 
					script-src 'nonce-${nonce}' 'unsafe-eval';">
				<title>Aeriocode</title>
			</head>
			<body>
	...
	protected async getHMRHtmlContent(): Promise<string> {
		...
		try {
			...
			if (process.env.IS_DEV) {
				HostProvider.window.showMessage({
					...
					message:
						"Aeriocode: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
				})
			}
		...
		return /*html*/ `
			...
			<html lang="en">
				<head>
					${process.env.IS_DEV ? '<script src="http://localhost:4083"></script>' : ""}
					<meta charset="utf-8">
					<meta name="viewport" content="width=device-width,initial-scale=1,shrink-to-fit=no">
