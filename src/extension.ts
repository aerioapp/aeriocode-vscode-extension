...
export async function activate(context: vscode.ExtensionContext) {
	...
	Logger.log("Aeriocode extension activated")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.mcpButtonClicked", (webview: any) => {
	...
	const openAeriocodeInNewTab = async () => {
		Logger.log("Opening Aeriocode in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "Aeriocode", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("aeriocode.popoutButtonClicked", openAeriocodeInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("aeriocode.openInNewTab", openAeriocodeInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.settingsButtonClicked", (webview: any) => {
			const isSidebar = !webview
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.historyButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.accountButtonClicked", (webview: any) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("Aeriocode dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				await vscode.commands.executeCommand("aeriocode.focusChatInput") // Ensure Aeriocode is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Aeriocode (Always available)
					const addAction = new vscode.CodeAction("Add to Aeriocode", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "aeriocode.addToChat",
						title: "Add to Aeriocode",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with Aeriocode (Always available)
					const explainAction = new vscode.CodeAction("Explain with Aeriocode", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: "aeriocode.explainCode",
						title: "Explain with Aeriocode",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Aeriocode (Always available)
					const improveAction = new vscode.CodeAction("Improve with Aeriocode", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: "aeriocode.improveCode",
						title: "Improve with Aeriocode",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Aeriocode (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Aeriocode", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: "aeriocode.fixWithAeriocode",
							title: "Fix with Aeriocode",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
				...
				const activeWebview = WebviewProvider.getLastActiveInstance()
				await activeWebview?.controller.fixWithAeriocode(selectedText, filePath, languageId, diagnostics)
				telemetryService.captureButtonClick("codeAction_fixWithAeriocode", activeWebview?.controller.task?.taskId)
			},
		),
	)

	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("aeriocode.focusChatInput") // Ensure Aeriocode is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getLastActiveInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("aeriocode.focusChatInput") // Ensure Aeriocode is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getLastActiveInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.focusChatInput", async () => {
			...
			if (activeWebview) {
				...
				if (tabInstances.length > 0) {
					...
					activeWebview = WebviewProvider.getSidebarInstance() || (await openAeriocodeInNewTab())
				}
			}
			...
			const clientId = activeWebview?.getClientId()
			if (!clientId) {
				console.error("FocusChatInput: Could not find or activate a Aeriocode webview to focus.")
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
					message: "Could not activate Aeriocode view. Please try opening it manually from the Activity Bar.",
				})
				return
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("aeriocode.generateGitCommitMessage", async (scm) => {
			await GitCommitGenerator?.generate?.(context, scm)
		}),
		vscode.commands.registerCommand("aeriocode.abortGitCommitMessage", () => {
	...
	context.subscriptions.push(
		context.secrets.onDidChange(async (event) => {
			if (event.key === "aeriocodeAccountId") {
				// Check if the secret was removed (logout) or added/updated (login)
				const secretValue = await context.secrets.get("aeriocodeAccountId")
	...
	return createAeriocodeAPI(sidebarWebview.controller)
}
...
export async function deactivate() {
	...
	Logger.log("Aeriocode extension deactivated")
}
