...
import type {
	...
	UsageTransaction,
	UserResponse,
} from "@shared/AeriocodeAccount"
import { AuthService } from "../auth/AuthService"
import { aeriocodeEnvConfig } from "@/config"

export class AeriocodeAccountService {
	private static instance: AeriocodeAccountService
	...
	/**
	 * Returns the singleton instance of AeriocodeAccountService
	 * @returns Singleton instance of AeriocodeAccountService
	 */
	public static getInstance(): AeriocodeAccountService {
		if (!AeriocodeAccountService.instance) {
			AeriocodeAccountService.instance = new AeriocodeAccountService()
		}
		return AeriocodeAccountService.instance
	}

	/**
	 * Returns the base URL for the Aeriocode API
	...
	/**
	 * Helper function to make authenticated requests to the Aeriocode API
	...
	/**
	 * Fetches usage statistics from the backend API
	 * @returns Usage statistics or undefined if failed
	 */
	async fetchUsageStatsRPC(): Promise<
		| {
				user_id: string
	...
	> {
		try {
			...
			console.error("Failed to fetch usage statistics (RPC):", error)
			return undefined
		}
	}
	 ...
	 * @returns User profile data or undefined if failed
	 */
	async fetchUserProfileRPC(): Promise<
		| {
				uid: string
				email?: string
				username?: string
	...
	> {
		try {
			console.log("DEBUG: Fetching user profile from /api/v1/users/profile")
			const data = await this.authenticatedRequest<{
				uid: string
				email?: string
				username?: string
			...
			console.log("DEBUG: Raw user profile response:", JSON.stringify(data, null, 2))
			return data
		} catch (error) {
			console.error("Failed to fetch user profile (RPC):", error)
			return undefined
		}
	}
}
