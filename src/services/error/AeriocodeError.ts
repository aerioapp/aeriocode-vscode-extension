...
export enum AeriocodeErrorType {
	Auth = "auth",
	Network = "network",
	RateLimit = "rateLimit",
	Balance = "balance",
...
export class AeriocodeError extends Error {
	readonly title = "AeriocodeError"
	readonly _error: ErrorDetails

	// Error details per providers:
	// Aeriocode: error?.error
	// Ollama: error?.cause
	// tbc
	constructor(
		raw: any,
	...
	/**
	 * Parses a stringified error into a AeriocodeError instance.
	 */
	static parse(errorStr?: string, modelId?: string): AeriocodeError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return AeriocodeError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a AeriocodeError instance.
	 * Always returns a AeriocodeError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): AeriocodeError {
		try {
			return new AeriocodeError(JSON.parse(error), modelId, providerId)
		} catch {
			return new AeriocodeError(error, modelId, providerId)
		}
	}

	public isErrorType(type: AeriocodeErrorType): boolean {
		return AeriocodeError.getErrorType(this) === type
	}
	...
	static getErrorType(err: AeriocodeError): AeriocodeErrorType | undefined {
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return AeriocodeErrorType.RateLimit
			}
		}
