...
import { AeriocodeAccountUserInfo } from "@/services/auth/AuthService"

/**
 * TelemetryService handles telemetry event tracking for the Aeriocode extension
...
export class TelemetryService {
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (!vscode.env.isTelemetryEnabled) {
			// Only show warning if user has opted in to Aeriocode telemetry but VS Code telemetry is disabled
			if (didUserOptIn) {
	...
	/**
	 * Records when aeriocode calls the task completion_result tool signifying that aeriocode is done with the task
