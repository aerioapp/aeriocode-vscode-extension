/**
 * AerioTempManager - Manages temporary files for AerioCode with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "aeriocode" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for AerioCode's temporary files.
 */
class AerioTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		const baseTempDir = os.tmpdir()
		const aeriocodeTempDir = path.join(baseTempDir, "aeriocode")

		try {
			fs.mkdirSync(aeriocodeTempDir, { recursive: true })
			this.tempDir = aeriocodeTempDir
		} catch {
	...
	/**
	 * Clean up old AerioCode temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the AerioCode temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(
					`AerioCode temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`,
				)
			}
		} catch (error) {
			Logger.error("Error during AerioCode temp cleanup", error)
		}
...
export const AerioTempManager = new AerioTempManagerImpl()
