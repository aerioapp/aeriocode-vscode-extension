...
import { getSavedApiConversationHistory, getSavedAeriocodeMessages } from "@core/storage/disk"
...
export function createTestServer(controller: Controller): http.Server {
	// Try to show the Aeriocode sidebar
	Logger.log("[createTestServer] Opening Aeriocode in sidebar...")
	vscode.commands.executeCommand("workbench.view.aeriocode-ActivityBar")

	// Then ensure the webview is focused/loaded
	vscode.commands.executeCommand("aeriocode.SidebarProvider.focus")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Aeriocode instance found" }))
					return
				}
				...
				try {
					...
					// TODO: convert apiKey to aeriocodeAccountId
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "aeriocode" as ApiProvider,
							aeriocodeAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.cacheService.setSecret("aeriocodeAccountId", apiKey)

						visibleWebview.controller.cacheService.setApiConfiguration(updatedConfig)

						// Update cache service to use aeriocode provider
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedAeriocodeMessages(visibleWebview.controller.context, taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Aeriocode messages: ${error}`)
						}
