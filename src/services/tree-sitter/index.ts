...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = aeriocodeIgnoreController ? aeriocodeIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, aeriocodeIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (aeriocodeIgnoreController && !aeriocodeIgnoreController.validateAccess(filePath)) {
		return null
	}
