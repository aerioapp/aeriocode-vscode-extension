...
export class SharedUriHandler {
	...
	public static async handleUri(uri: vscode.Uri): Promise<boolean> {
		...
		try {
			switch (path) {
				case "/auth":
				case "/api/auth/sso-callback": {
					console.log("SharedUriHandler: Auth callback received:", { path: uri.path, provider: query.get("provider") })

					const token = query.get("session_id") || query.get("idToken") || query.get("token")
					const provider = query.get("provider") || "backend"

					if (token) {
						await visibleWebview.controller.handleAuthCallback(token, provider)
						return true
					}
					console.warn("SharedUriHandler: Missing idToken/token parameter for auth callback")
					return false
				}
				default:
