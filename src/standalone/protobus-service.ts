...
export const PROTOBUS_PORT = 4079
export const HOSTBRIDGE_PORT = 4080
...
function getProtobusServiceNames(packageDefinition: { [x: string]: any }): string[] {
	// Filter service names to only include aeriocode services
	const protobusServiceNames = Object.keys(packageDefinition).filter(
		(name) => name.startsWith("aeriocode.") || name.startsWith("grpc.health"),
	)
