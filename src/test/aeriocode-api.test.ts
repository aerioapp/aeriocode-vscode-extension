...
import type { AeriocodeAPI } from "../exports/aeriocode"
import { setVscodeHostProviderMock } from "./host-provider-test-utils"

describe("AeriocodeAPI Core Functionality", () => {
	let api: AeriocodeAPI
	...
	beforeEach(async () => {
		...
		// This is needed because the real createAeriocodeAPI uses it for getCustomInstructions
		getGlobalStateStub = sandbox.stub(stateModule, "getGlobalState")

		// Create a mock controller that matches what the real createAeriocodeAPI expects
		...
		// Create API instance
		api = createAeriocodeAPI(mockController)
	})
