...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/AeriocodeAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { AeriocodeDataMock } from "./data"

const E2E_API_SERVER_PORT = 4082

export const MOCK_AERIOCODE_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

export class AeriocodeApiServerMock {
	static globalSharedServer: AeriocodeApiServerMock | null = null
	...
	public readonly API_USER = new AeriocodeDataMock("personal")
	...
	public static async startGlobalServer(): Promise<AeriocodeApiServerMock> {
		if (AeriocodeApiServerMock.globalSharedServer) {
			return AeriocodeApiServerMock.globalSharedServer
		}
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (isAuthRequired && authToken) {
				...
				const user = AeriocodeApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				AeriocodeApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = AeriocodeApiServerMock.matchRoute(path, method)
				...
				const controller = AeriocodeApiServerMock.globalSharedServer!
								...
								choices: [
									{
										...
										message: {
											role: "assistant",
											content: "Hello! I'm a mock Aeriocode API response.",
										},
										finish_reason: "stop",
									},
								],
		...
		const controller = new AeriocodeApiServerMock(server)
		AeriocodeApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			AeriocodeApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				AeriocodeApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					console.log(`AeriocodeApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = AeriocodeApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		AeriocodeApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		AeriocodeApiServerMock.globalSockets.clear()

		await new Promise<void>((resolve) => {
			server.close(() => resolve())
		})

		AeriocodeApiServerMock.globalSharedServer = null
	}
}
