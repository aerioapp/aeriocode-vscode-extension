...
teardown("cleanup test environment", async () => {
	AeriocodeApiServerMock.stopGlobalServer()
		.then(() => console.log("AeriocodeApiServerMock stopped successfully."))
		.catch((error) => console.error("Error stopping AeriocodeApiServerMock:", error))
