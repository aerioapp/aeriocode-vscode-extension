...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "aeriocode-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .aeriocoderules/workflows directory specifically", async () => {
		// Create a test directory structure
		const aeriocoderulesDirTest = path.join(tmpDir, "aeriocoderules-test")
		const aeriocoderulesDirPath = path.join(aeriocoderulesDirTest, ".aeriocoderules")

		// Create .aeriocoderules directory and root files
		await fs.mkdir(aeriocoderulesDirPath, { recursive: true })
		await fs.writeFile(path.join(aeriocoderulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(aeriocoderulesDirPath, "settings.js"), "// settings")

		// Create .aeriocoderules/other directory and files
		const otherDirPath = path.join(aeriocoderulesDirPath, "other")
		...
		// Create .aeriocoderules/workflows directory and files
		const workflowsDirPath = path.join(aeriocoderulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(aeriocoderulesDirPath)
		...
		const filteredFiles = await readDirectory(aeriocoderulesDirPath, [[".aeriocoderules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(aeriocoderulesDirPath, "config.json"),
			path.resolve(aeriocoderulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(aeriocoderulesDirPath, [
			[".aeriocoderules", "workflows"],
			[".aeriocoderules", "other"],
		])
