...
import {
	AeriocodeRulesToggles,
	RefreshedRules,
	ToggleAeriocodeRuleRequest,
	ToggleCursorRuleRequest,
	ToggleWindsurfRuleRequest,
	ToggleWorkflowRequest,
} from "@shared/proto/aeriocode/file"
...
const AeriocodeRulesToggleModal: React.FC = () => {
	const {
		...
		setGlobalAeriocodeRulesToggles,
		setLocalAeriocodeRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalAeriocodeRulesToggles?.toggles) {
						setGlobalAeriocodeRulesToggles(response.globalAeriocodeRulesToggles.toggles)
					}
					if (response.localAeriocodeRulesToggles?.toggles) {
						setLocalAeriocodeRulesToggles(response.localAeriocodeRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalAeriocodeRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localAeriocodeRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleAeriocodeRule(
			ToggleAeriocodeRuleRequest.create({
				isGlobal,
				rulePath,
				enabled,
			}),
			...
			.catch((error) => {
				console.error("Error toggling Aeriocode rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: AeriocodeRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div ref={modalRef}>
			<div ref={buttonRef} className="inline-flex min-w-0 max-w-full">
				<Tooltip tipText="Manage Aeriocode Rules & Workflows" visible={isVisible ? false : undefined}>
					<VSCodeButton
						appearance="icon"
						aria-label="Aeriocode Rules"
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide Aeriocode with system-level guidance. Think of them as a persistent way
								to include context and preferences for your projects or globally for every conversation.{" "}
								<VSCodeLink
									href="https://docs.aeriocode.bot/features/aeriocode-rules"
							...
							<p>
								Workflows allow you to define a series of steps to guide Aeriocode through a repetitive set of
								...
								in the chat.{" "}
								<VSCodeLink
									href="https://docs.aeriocode.bot/features/slash-commands/workflows"
									style={{ display: "inline" }}
									className="text-xs">
									Docs
								</VSCodeLink>
...
export default AeriocodeRulesToggleModal
