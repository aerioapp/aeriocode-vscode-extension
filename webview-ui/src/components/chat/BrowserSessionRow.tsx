...
import { BrowserAction, BrowserActionResult, AeriocodeMessage, AeriocodeSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: AeriocodeMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: AeriocodeMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: AeriocodeMessage[] = []
		let nextActionMessages: AeriocodeMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					<>{isAutoApproved ? "Aeriocode is using the browser:" : "Aeriocode wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{
