...
import { FileSearchRequest, RelativePathsRequest } from "@shared/proto/aeriocode/file"
import { UpdateApiConfigurationRequest } from "@shared/proto/aeriocode/models"
import { PlanActMode, TogglePlanActModeRequest } from "@shared/proto/aeriocode/state"
...
const ModelSelectorTooltip = styled.div<ModelSelectorTooltipProps>`
	...
	padding: 15px;
	border-radius: 3px;
	z-index: 10000; // Significantly increased z-index to ensure it appears above all other elements
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const { filePaths, mode, apiConfiguration, platform, localWorkflowToggles, globalWorkflowToggles } = useExtensionState()
		...
		const submitApiConfig = useCallback(async () => {
			...
			// Aeriocode-only mode - no openRouterModels validation needed
			const modelIdValidationResult = null
		...
		// Get model display name (Aeriocode only)
		const modelDisplayName = useMemo(() => {
			...
			// Format the model ID to display name
			...
			<div>
				...
				<ControlsContainer>
					...
					<Tooltip
						...
						tipText={`In ${shownTooltipMode === "act" ? "Act" : "Plan"}  mode, Aeriocode will ${shownTooltipMode === "act" ? "complete the task immediately" : "gather information to architect a plan"}`}
