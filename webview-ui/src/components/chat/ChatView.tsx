...
import type { AeriocodeApiReqInfo, AeriocodeMessage } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { BooleanRequest, EmptyRequest, StringRequest } from "@shared/proto/aeriocode/common"
...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	...
	const isProdHostedApp = userInfo?.apiBaseUrl === "https://app.aeriocode.bot"
	const shouldShowQuickWins = isProdHostedApp && (!taskHistory || taskHistory.length < QUICK_WINS_HISTORY_THRESHOLD)

	//const task = messages.length > 0 ? (messages[0].say === "task" ? messages[0] : undefined) : undefined) : undefined
	const task = useMemo(() => messages.at(0), [messages]) // leaving this less safe version here since if the first message is not a task, then the extension is in a bad state and needs to be debugged (see Aeriocode.abort)
	...
	const lastApiReqTotalTokens = useMemo(() => {
		const getTotalTokensFromApiReqMessage = (msg: AeriocodeMessage) => {
			...
			const { tokensIn, tokensOut, cacheWrites, cacheReads }: AeriocodeApiReqInfo = JSON.parse(msg.text)
	...
	// Use streaming hook
	const isStreaming = useIsStreaming(modifiedMessages, aeriocodeAsk, enableButtons, primaryButtonText)
	...
	useEffect(() => {
		const clientId = (window as { aeriocodeClientId?: string }).aeriocodeClientId
		if (!clientId) {
