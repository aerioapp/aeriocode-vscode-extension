...
const ACTION_STYLES = {
	Add: { icon: FilePlus, iconClass: "text-[var(--vscode-charts-green)]", borderClass: "border-l-[var(--vscode-charts-green)]" },
	Delete: {
		icon: FileX,
		iconClass: "text-[var(--vscode-errorForeground)]",
		borderClass: "border-l-[var(--vscode-errorForeground)]",
	},
	default: {
		icon: FileText,
		iconClass: "text-[var(--vscode-textLink-foreground)]",
		borderClass: "border-l-[var(--vscode-editor-background)]",
	},
} as const
...
const FileBlock = memo<{ file: Patch; isStreaming: boolean; startLineNumber?: number }>(
	({ file, isStreaming, startLineNumber }) => {
		...
		return (
			<div className="bg-[var(--vscode-textCodeBlock-background)] rounded-xs border border-[var(--vscode-editorGroup-border)] overflow-hidden">
				<button
					className="w-full flex items-center gap-2 p-2 bg-[var(--vscode-textCodeBlock-background)] transition-colors justify-between cursor-pointer border-none"
					...
					<div className="flex items-center gap-3 flex-1 w-full overflow-hidden">
						<div className={cn("flex items-center gap-2 w-full", actionStyle.borderClass)}>
							...
							<span
								className="font-medium truncate hover:underline hover:text-[var(--vscode-textLink-foreground)]"
					...
					<div className="flex items-center gap-2">
						...
						<span
							className="p-1 hover:bg-[var(--vscode-descriptionForeground)]/20 rounded-xs transition-colors"
							onClick={handleOpenFile}
							title="Open file in editor">
							<SquareArrowOutUpRightIcon className="size-2 text-[var(--vscode-descriptionForeground)] hover:text-[var(--vscode-foreground)]" />
						</span>
					</div>
				</button>
				...
				{isExpanded && (
					<div
						className="border-t border-[var(--vscode-textCodeBlock-background)] max-h-80 overflow-y-auto overflow-x-auto"
...
const DiffStats = memo<{ additions: number; deletions: number }>(({ additions, deletions }) => (
	<div className="text-xs text-gray-500 flex">
		{additions > 0 && <span className="text-[var(--vscode-charts-green)]">+{additions}</span>}
		{additions > 0 && deletions > 0 && <span className="mx-1">·</span>}
		{deletions > 0 && <span className="text-[var(--vscode-errorForeground)]">-{deletions}</span>}
	</div>
))
		...
		return (
			<div
				...
				{showLineNumberColumn && (
					<span
						className={cn(
							"w-10 min-w-10 text-right pr-2 py-0.5 select-none border-r border-[var(--vscode-textCodeBlock-background)]/50",
							isAddition && "text-green-400/60",
							isDeletion && "text-red-400/60",
							!isAddition && !isDeletion && "text-[var(--vscode-descriptionForeground)]/50",
						)}>
						{lineNumber ?? ""}
					</span>
				)}
				...
				<span
					className={cn(
						...
						isAddition && "text-green-400",
						isDeletion && "text-red-400",
						!isAddition && !isDeletion && "text-[var(--vscode-editor-foreground)]",
					)}>
					{code}
				</span>
			</div>
