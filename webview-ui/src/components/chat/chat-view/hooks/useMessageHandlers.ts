...
import { AeriocodeMessage, AeriocodeAsk } from "@shared/ExtensionMessage"
...
import { AskResponseRequest, NewTaskRequest } from "@shared/proto/aeriocode/task"
 ...
 */
export function useMessageHandlers(messages: AeriocodeMessage[], chatState: ChatState, isStreaming: boolean): MessageHandlers {
	const {
		inputValue,
		setInputValue,
		activeQuote,
		...
		async (text: string, images: string[], files: string[]) => {
			...
			if (hasContent) {
				...
				if (messages.length === 0) {
					...
					// Fallback: aeriocodeAsk is undefined (e.g., during race window after task
					// completion). Send as askResponse so the backend can process it if waiting.
					console.warn("[ChatView] aeriocodeAsk is undefined, sending as fallback askResponse")
					await TaskServiceClient.askResponse(
						AskResponseRequest.create({
							responseType: "messageResponse",
							text: messageToSend,
		...
		async (text?: string, images?: string[], files?: string[]) => {
			...
			switch (aeriocodeAsk) {
				...
				case "resume_completed_task":
					...
					break
				case "new_task":
					console.info("new task button clicked!", { lastMessage, messages, aeriocodeAsk, text })
