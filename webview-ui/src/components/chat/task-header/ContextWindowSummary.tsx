...
const AccordionItem = memo<{
	...
	return (
		<div className="flex flex-col w-full">
			<div
				className="flex justify-between items-center gap-1 cursor-pointer hover:bg-[var(--vscode-foreground)]/5 rounded p-0.5 transition-colors w-full"
				onClick={handleClick}>
				<div className="flex items-center gap-1">
					{isExpanded ? <ChevronDownIcon size={12} /> : <ChevronRightIcon size={12} />}
					<div className="font-semibold">{title}</div>
				</div>
				<div style={{ color: "var(--vscode-descriptionForeground)" }}>{value}</div>
			</div>
			{isExpanded && children && (
				<div className="ml-5 my-1 text-xs" style={{ color: "var(--vscode-descriptionForeground)" }}>
					{children}
				</div>
			)}
		</div>
...
export const ContextWindowSummary: React.FC<TaskContextWindowButtonsProps> = ({
	...
	return (
		<div
			className="context-window-tooltip-content flex flex-col gap-2 rounded shadow-sm z-100 w-60 p-1"
			style={{ backgroundColor: "var(--vscode-menu-background)" }}>
			{autoCompactThreshold > 0 && (
				<AccordionItem
					...
					value={
						<span
							...
							}}>{`${(autoCompactThreshold * 100).toFixed(0)}%`}</span>
					}>
					<div className="space-y-1">
