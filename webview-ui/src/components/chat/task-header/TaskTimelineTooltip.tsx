...
const TaskTimelineTooltip = ({ message, children }: TaskTimelineTooltipProps) => {
	const getMessageDescription = (message: AeriocodeMessage): string => {
		if (message.type === "say") {
			switch (message.say) {
	...
	const getMessageContent = (message: AeriocodeMessage): string => {
		if (message.text) {
	...
	const getTimestamp = (message: AeriocodeMessage): string => {
		if (message.ts) {
