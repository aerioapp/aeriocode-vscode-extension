...
import { CheckpointRestoreRequest } from "@shared/proto/aeriocode/checkpoints"
import { Int64Request } from "@shared/proto/aeriocode/common"
import { AeriocodeCheckpointRestore } from "@shared/WebviewMessage"
...
export const CheckmarkControl = ({ messageTs, isCheckpointCheckedOut }: CheckmarkControlProps) => {
	...
	const handleRestoreTask = async () => {
		...
		try {
			const restoreType: AeriocodeCheckpointRestore = "task"
	...
	const handleRestoreWorkspace = async () => {
		...
		try {
			const restoreType: AeriocodeCheckpointRestore = "workspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
	...
	const handleRestoreBoth = async () => {
		...
		try {
			const restoreType: AeriocodeCheckpointRestore = "taskAndWorkspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
