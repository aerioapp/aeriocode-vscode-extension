...
// Simple ModelDescriptionMarkdown component for Aeriocode-only configuration
const ModelDescriptionMarkdown = ({ markdown, isExpanded, setIsExpanded, isPopup }: any) => {
	const shouldTruncate = markdown.length > 200 && !isExpanded
	const displayText = shouldTruncate ? markdown.substring(0, 200) + "..." : markdown

	return (
		<div>
			<span style={{ fontWeight: 500 }}>Description:</span> {displayText}
			{markdown.length > 200 && (
				<button
